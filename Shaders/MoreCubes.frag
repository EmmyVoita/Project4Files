...
void main()
{
	...
	vec3 preturbedNormal = normalize(vs_normal + (normal * 0.08));
	...
	color = texture(material.diffuseTex, vs_texcoord) * ((vec4(ambientFinal, 1.0) + (1.0-shadow) * (vec4(diffuseFinal.xyz, 1.0) + vec4(specularFinal.xyz, 1.0))) * pointLight.intensity); 
	//color = texture(material.diffuseTex, vs_texcoord) * ((vec4(ambientFinal, 1.0) +  (vec4(diffuseFinal.xyz, 1.0) + vec4(specularFinal.xyz, 1.0))) * pointLight.intensity); 
	color = vec4(color.x,color.y,color.z,0.9);
